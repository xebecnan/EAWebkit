...
void PNGImageDecoder::rowAvailable(unsigned char* rowBuffer, unsigned rowIndex, int interlacePass)
{
    ...
    if(cachePrune == false) {
        if(!overflowFlag) {
            char buffer[512];
            sprintf(buffer, "Non Fatal Warning: Ram cache overflow detected in PNG image decoding of %d bytes. You may want to increase the Ram Cache otherwise performance might drop.\n", (size<< 2) );
