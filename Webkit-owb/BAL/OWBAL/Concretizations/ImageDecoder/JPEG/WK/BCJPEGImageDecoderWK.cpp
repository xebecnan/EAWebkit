...
bool JPEGImageDecoder::outputScanlines()
{
    ...
    if((buffer.status() == RGBA32Buffer::FrameEmpty) ||(size != curSize) ) {
        ...
        if(cachePrune == false){
            if(!overflowFlag){
                char buffer[512];
                sprintf(buffer, "Non Fatal Warning: Ram cache overflow detected in JPEG image decoding of %d bytes. You may want to increase the Ram Cache otherwise performance might drop.\n", (size<< 2) );
