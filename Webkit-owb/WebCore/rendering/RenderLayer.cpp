                        ...
                        RenderObject* paintingRoot, bool appliedTransform)
{
    ...
    if (m_transform && !appliedTransform) {
        ...
        // 2/24/11 EA - CSidhall - If the cliprect is null, we don't want to draw it as the context setClip will
        // reset the clip rect to the full surface if the passed cliprect is empty (unless the drawDisable flag is set).
        // Seems it is better to just abort the draw here since the clip rect is NULL.  We save some time.
        // This bug occured with very large textures when the parent node is off screen or not included in the dirtyrect that is being refreshed.
        if(clipRect.isEmpty())
            return;
