                                       ...
                                       int tx, int ty, int w, int h, InlineFlowBox* box, CompositeOperator op)
{
    ...
    // 2/11/11 EA - YC Chin proposed this change for a background alpha problem.
    // "If you have a body with alpha between 0 and 1, it would treat it as a solid background instead of transparent even if the passed in flag says transparent."
    // Old code: if (!bgLayer->next() && isRoot() && !(bgColor.isValid() && bgColor.alpha() > 0) && view()->frameView()) { 
    if (!bgLayer->next() && isRoot() && !(bgColor.isValid() && bgColor.alpha() == 255) && view()->frameView()) {
